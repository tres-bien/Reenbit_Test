    ...
    public class UnitTest1
    {
        ...
        public class SendEmailFunctionTests
        {
            ...
            public async Task Run_ValidRequest_ReturnsOkResult()
            {
                ...
                var smtpClientMock = new Mock<ISmtpClient>();
                smtpClientMock.Setup(s => s.ConnectAsync(It.IsAny<string>(), It.IsAny<int>(), It.IsAny<SecureSocketOptions>(), CancellationToken.None))
                    .Returns(Task.CompletedTask);
                smtpClientMock.Setup(s => s.AuthenticateAsync(It.IsAny<string>(), It.IsAny<string>(), CancellationToken.None))
                    .Returns(Task.CompletedTask);
                smtpClientMock.Setup(s => s.SendAsync(It.IsAny<MimeMessage>(), CancellationToken.None))
    .Returns(Task.CompletedTask);

                smtpClientMock.Setup(s => s.DisconnectAsync(true, CancellationToken.None))
                    .Returns(Task.CompletedTask); // Set up DisconnectAsync
